...
const tokenStorage = {
    ...
    print() {
        console.log("Current token storage:", JSON.stringify(this.storage, null, 2));
    }
};
...
function logTokenStorage() {
    tokenStorage.print();
    // console.log("Current token storage:", JSON.stringify(Array.from(tokenStorage.entries()), null, 2));
}
...
async function exchangeCodeForTokens(clientId, clientSecret, code) {
    try {
        ...
        if ("error" in response.data) {
            console.error("❌ Error in token exchange response:", response.data.error);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    // console.log("CURRENT STORAGE:", JSON.stringify(tokenStorage, null, 2));
