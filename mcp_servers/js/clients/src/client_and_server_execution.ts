		...
		if (selected_client == "MCP_CLIENT_AZURE_AI") {
			...
			if (initialLlmResponse.Status == false) {
				if (streaming_callback?.is_stream) {
					streaming_callback?.streamCallbacks.onData(
						JSON.stringify({
							Data: initialLlmResponse.Error,
							Error: initialLlmResponse?.Error?.message || initialLlmResponse?.Error,
							Status: false,
				...
				ClientAndServerExecution.Error =
					initialLlmResponse?.Error?.message || initialLlmResponse?.Error;
			...
			ClientAndServerExecution.Data.total_input_tokens +=
				initialLlmResponse.Data?.total_input_tokens || 0;
			ClientAndServerExecution.Data.total_output_tokens +=
				initialLlmResponse.Data?.total_output_tokens || 0;
			ClientAndServerExecution.Data.final_llm_response =
				initialLlmResponse.Data?.final_llm_response;
			ClientAndServerExecution.Data.llm_responses_arr.push(
				initialLlmResponse.Data?.final_llm_response
			);
			...
			if (extractedResult.isFunctionCall) {
				...
				for (const tool_name of extractedResult.selectedTools) {
					...
					const matchingTool = parsedTools.find(
						(temp_tool: any) => temp_tool?.function?.name == tool_name
					);
					if (matchingTool) {
				...
				while (true) {
					...
					ClientAndServerExecution.Data.total_input_tokens +=
						response.Data?.total_input_tokens || 0;
					ClientAndServerExecution.Data.total_output_tokens +=
					...
					for (var tool of response.Data?.final_llm_response?.choices?.[0]?.message?.tool_calls ||
						...
						var tool_call_result: any = await CallAndExecuteTool(
							selected_server,
						...
						if (streaming_callback?.is_stream) {
							streaming_callback?.streamCallbacks.onData(
								JSON.stringify({
									Data: `${selected_server} MCP server ${toolName} call result  : ${JSON.stringify(
										tool_call_result
									)}`,
									Error: null,
									Status: true,
						...
						let tool_call_content_data = `Calling tool: ${toolName} with arguments: ${JSON.stringify(
							args
						)} and result: ${JSON.stringify(tool_call_result)}`;
						client_details.chat_history.push({
							role: "assistant",
				...
				client_details.prompt = `${temp_prompt}. Available tools: ${JSON.stringify(
				...
				ClientAndServerExecution.Data.total_input_tokens +=
					normalResponse.Data?.total_input_tokens || 0;
				ClientAndServerExecution.Data.total_output_tokens +=
					normalResponse.Data?.total_output_tokens || 0;
				ClientAndServerExecution.Data.final_llm_response = normalResponse.Data?.final_llm_response;
				ClientAndServerExecution.Data.llm_responses_arr.push(
					normalResponse.Data?.final_llm_response
				);
				...
				if (
					...
					(normalResponse.Data?.final_llm_response?.choices?.[0]?.message?.content != null &&
						normalResponse.Data?.final_llm_response?.choices?.[0]?.message?.content != "")
				) {
				...
				if (
					(normalResponse.Data?.final_llm_response?.choices?.[0]?.message?.tool_calls || [])
						.length > 0
				) {
					var final_tool_calls = [];
					for (const tool_name of extractedResult.selectedTools) {
			...
			if (extractedResult.isFunctionCall) {
				...
				while (true) {
					...
					for (const part of response.Data?.final_llm_response?.candidates?.[0]?.content?.parts ||
						...
						const tool_call_result: any = await CallAndExecuteTool(
							selected_server,
							selected_server_credentials,
							toolName,
							args
						...
						const tool_call_content_data = `Calling tool: ${toolName} with arguments: ${JSON.stringify(
							args
						)} and result: ${JSON.stringify(tool_call_result)}`;
						client_details.chat_history.push({
							role: "model",
				...
				if (
					!normalResponse.Status ||
					(normalResponse.Data?.final_llm_response?.candidates?.[0]?.content?.parts?.[0]?.text !=
						null &&
						normalResponse.Data?.final_llm_response?.candidates?.[0]?.content?.parts?.[0]?.text !=
							"")
				) {
				...
				if (
					(
						normalResponse.Data?.final_llm_response?.candidates?.[0]?.content?.parts[0]
							?.functionCall || []
					).length > 0
				) {
					var final_tool_calls = [];
...
function extractDataFromResponse(responseContent: string): {
	...
	try {
		...
		if (selectedToolsMatch && selectedToolsMatch[1]) {
			...
			result.selectedTools = selectedToolsMatch[1]
				.split(",")
				.map((tool) => tool.trim())
...
async function CallAndExecuteTool(
	selected_server: any,
	server_credentials: any,
	tool_name: any,
	args: any
) {
	...
	switch (selected_server) {
		case "CONFLUENCE":
			const confluenceCreds =
				server_credentials[selected_server]?.credentials ||
				server_credentials[selected_server] ||
				{};
		...
		case "G_DRIVE":
			...
			args["__credentials__"] =
				server_credentials[selected_server]?.web || server_credentials[selected_server] || {};
			break;
		case "SALESFORCE_MCP":
			args.username = server_credentials[selected_server]?.username || "";
			args.password = server_credentials[selected_server]?.password || "";
		...
		case "ZOHOMCP":
			args["__credentials__"] = {
				...
				client_secret: server_credentials[selected_server]?.client_secret || "",
				authorization_code:
					server_credentials[selected_server]?.authorization_code ||
					server_credentials[selected_server]?.autharization_code ||
					"",
			};
			break;
		case "FIGMA_MCP":
		...
		case "VERCEL":
			args["__credentials__"] = {
				token: server_credentials[selected_server]?.token || "",
				teamId: server_credentials[selected_server]?.teamId || "",
				projectId: server_credentials[selected_server]?.projectId || "",
				deploymentId: server_credentials[selected_server]?.deploymentId || "",
				GEMINI_API_KEY: server_credentials[selected_server]?.GEMINI_API_KEY || "",
			};
			break;
		default:
			break;
